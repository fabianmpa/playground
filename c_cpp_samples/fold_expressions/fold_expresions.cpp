...
//C++17 style fold expression with constexpr if addition to make more generic
template <typename ... Ts>
...
int main()
{
    int suma {sum_rec(1,2,3,4,5)};
    std::string word{sum_generic("Hello","world")};
    ...
    assert( bitwise_or(1,2,3,4) == 7);
    assert( bitwise_or() == 0);

    assert(within(0,9,1,2,3,4,5) == 7);
